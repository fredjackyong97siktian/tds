...
def _repair_grouping_with_gemini(
    ...
    *,
    ...
    prompt = (
        ...
        "walking to the left is an exit. Judge this from the sequence of frames for that trigger, not a single still frame. "
...
def _run_gemini_grouping_for_batch(db: Session, *, batch_id: int, script_run_id: int) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    while pending_triggers:
        ...
        prompt = (
            ...
            "This camera has a fixed orientation: a customer facing right and walking to the right is an entry. A customer facing left and "
            "walking to the left is an exit. Judge this from the sequence of frames for that trigger, not a single still frame - compare the "
            "person's position and facing direction across the frames to determine which way they are actually moving. "
            "If clear walking direction conflicts with simple timestamp assumptions, prioritize the walking direction. "
