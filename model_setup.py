...
def configure_detect_model_env():
    ...
    _set_if_exists("MODEL_PATH", DETECTION_DIR / "yolo26s.pt")
    _set_if_exists("TRACKER_PATH", TRACKER_DIR / "custom_tracker.yaml")
    _set_if_exists("REID_MASK_MODEL_PATH", SEGMENTATION_DIR / "yoloe-11l-seg.pt")
    _set_if_exists("REID_WEIGHTS_PATH", REID_DIR / "osnet_x1_0_msmt17.pt")

    # Kiosk-side env names used by Detect.py
    _set_if_exists("KIOSK_PERSON_MODEL_PATH", DETECTION_DIR / "yolo26s.pt")
    _set_if_exists("KIOSK_PRODUCT_MODEL_PATH", DETECTION_DIR / "yolo26s.pt")
    _set_if_exists("KIOSK_TRACKER_PATH", TRACKER_DIR / "custom_tracker.yaml")
    _set_if_exists("KIOSK_POSE_MODEL_PATH", POSE_DIR / "yolo26l-pose.pt")
    ...
    _set_if_exists("MOBILECLIP_PT_PATH", TEXT_DIR / "mobileclip_blt.pt")
