...
-- Blocking is not a table TDS owns: the door-entry system's own
-- phonenumber/fingerprint tables carry a `status` column (ACCESS/BLOCKED,
-- plus DEMANDING for cards) that it already checks itself - see
-- repositories.list_blocked_entries / update_entry_status.
...
CREATE TABLE IF NOT EXISTS sesamedb.tds_video_asset (
    ...
    CONSTRAINT chk_video_asset_status
        CHECK (status IN ('not_retrieved', 'retrieving', 'frames_retrieved', '10_frames_retrieved', 'ready', 'processing', 'processed', 'deleted', 'issue'))
);
...
CREATE TABLE IF NOT EXISTS sesamedb.tds_trigger_frame (
    ...
    is_best_for_verification TINYINT(1) NOT NULL DEFAULT 0,
    created_at TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP,
...
CREATE TABLE IF NOT EXISTS sesamedb.tds_script_run (
    ...
    UNIQUE KEY uq_script_run_runner_job_id (runner_job_id),
    ...
    CONSTRAINT chk_script_run_name
        CHECK (script_name IN ('retrieve_video', 'entry', 'kiosk', 'kiosk_match', 'grouping', 'carry_confidence')),
...
-- theft. Never triggered automatically - see mysql_theft_action.sql.
...
CREATE TABLE IF NOT EXISTS sesamedb.tds_filter_grouping_batch (
    ...
    CONSTRAINT chk_filter_grouping_batch_status
        CHECK (status IN ('pending', 'dispatching', 'running', 'success', 'failed', 'issue', 'missed', 'avoid'))
);
